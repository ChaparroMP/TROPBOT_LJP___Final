...
if (!empty($_POST['email']) && !empty($_POST['password'])) {
    ...
    $records = $conn->prepare('SELECT id, firstName, lastName, email, password FROM login_users WHERE email = :email');
    ...
    if (($results != 0) && password_verify($_POST['password'], $results['password'])) {
      ...
      echo '
         <script>
            alert("¡BIENVENIDO!");
            window.location = "http://tropbot-ljp-tg.125mb.com/usersLogin/login.php";
         </script>';
      exit();
...
<html>
  ...
  <title>TROPBOT TG || Porque sabemos que amas tu cultivo  | Iniciar Sesión </title>
