...
import sys, time, serial, csv, os, smbus, zipfile, glob
import ADS1015V3
...
## ------------------- Función de Comprimir archivos --------------------------##
# Esta función sale del ejemplo:https://linuxhint.com/python_zip_file_directory/
...
while True:
 ...
 #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        while(adquirir_completo==0):
            ...
            #/////////////////////////////////////////////////////////////////////////
            elif(state_adq==4):# Tomar foto con la cámara
                camera.start_preview()
                hour_Foto =  time.strftime("%Y-%m-%d_%H-%M-%S")
                camera.capture('/home/pi/Img/image_%s.jpg' % hour_Foto) 
                camera.stop_preview()
                state_adq =5
                state_general = 3
 ...
 #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    
 #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~        
    elif(state_general==7): #Manejo de imágenes
        if check_connection():
            GPIO.output(LED_internet, True)
            #------------------------------------------------------------------ 
            # Guardar imágenes en archivo comprimido

            
            dir_name = '/home/pi/Img'
            # Llamar a la función para recuperar todos los archivos y carpetas del directorio asignado 
            filePaths = retrieve_file_paths(dir_name)
            # Mandar las imágenes a un zipfile
            zip_file = zipfile.ZipFile('/Img/Imgs_actuales.zip', 'w')
            with zip_file:
            ...
            #------------------------------------------------------------------
            # Eliminar archivo actual de Dropbox 
            dbx.files_delete("/Img_Recorrido/Actual/Imgs_actuales.zip")
            print("Archivo de imag actual eliminado")
            
            #------------------------------------------------------------------
            # Subir archivos a DROPBOX
            print("Enviando información a Imágenes a DROPBOX")
            hour_Foto =  time.strftime("%Y-%m-%d_%H-%M-%S")
            with open("/home/pi/Img/Imgs_actuales.zip", "rb") as f_file:
                dbx.files_upload(f_file.read(), "/Img_Recorrido/Historico/Imgs_%s.zip"% hour_Foto, mute = True)
                f_file.close()

            with open("/home/pi/Img/Imgs_actuales.zip", "rb") as f_filew:
                dbx.files_upload(f_filew.read(), "/Img_Recorrido/Actual/Imgs_actuales.zip", mute = True)
                f_filew.close()
            print("Envio exitoso de imágenes")

            #------------------------------------------------------------------
            # Eliminar archivos de Raspberry (locales)
            py_files = glob.glob('/home/pi/Img/*.jpg')
            for py_file in py_files:
                try:
                    os.remove(py_file)
                except OSError as e:
                    print(f"Error:{ e.strerror}")
            os.remove("/home/pi/Img/Imgs_actuales.zip")
            print("Se eliminan los archivos de la carpeta Img")
